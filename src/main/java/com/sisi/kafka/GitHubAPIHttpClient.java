...
public class GitHubAPIHttpClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(
            GitHubAPIHttpClient.class
    );

    // for efficient http rquests
    ...
    protected JSONArray getNextIssues(Integer page, Instant since) throws InterruptedException {
        ...
        try {
            ...
            XRateLimit = Integer.valueOf(headers.getFirst(
                    "X-RateLimit-Limit"
            ));
            XRateRemaining = Integer.valueOf(headers.getFirst(
                    "X-RateLimit-Remaining"
            ));
            XRateReset = Integer.valueOf(headers.getFirst(
                    "X-RateLimit-Reset"
            ));
            ...
            switch(jsonResponse.getStatus()){
                ...
                case 401:
                    throw new ConnectException(
                            "Bad GitHub credentials provided, please edit your config"
                    );

                case 403:
                    log.info(jsonResponse.getBody().getObject()
                    .getString("message"));
                    log.info(String.format("rate limit : %s",
                            XRateLimit));
                    log.info(String.format("remaining calls : ",
                            XRateRemaining));
                    log.info(String.format("limit will reset at %s",
                            LocalDateTime.ofInstant(
                                    Instant.ofEpochSecond(XRateReset),
                ...
                default:
                    ...
                    log.error(
                            String.valueOf(jsonResponse.getStatus())
                    );
                    ...
                    Thread.sleep(5000L);
                    return getNextIssues(page,since); // re-try
            }
    ...
    protected HttpResponse<JsonNode> getNextIssuesAPI(
            Integer page, Instant since) throws UnirestException {

        GetRequest unirest = Unirest.get(
                constructUrl(page, since)
        );

        if (!config.getAuthUsername().isEmpty() &&
                !config.getAuthPassword().isEmpty()) {

            unirest = unirest.basicAuth(config.getAuthUsername(),
    ...
    public void sleep() throws InterruptedException {
        ...
        long sleepTime = (long) Math.ceil(
                (double) (XRateReset - Instant.now().getEpochSecond()) /
                        XRateRemaining);
    ...
    public void sleepIfNeed() throws InterruptedException {
        ...
        if (XRateRemaining <= 10 && XRateRemaining > 0) {
            log.info(String.format("approaching limit soon, %s requests left",
                    XRateRemaining));
            sleep();
        }
    }
